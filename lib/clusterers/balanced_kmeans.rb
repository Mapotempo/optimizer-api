      ...
      def update_metrics(data_item, cluster_index)
        @unit_symbols.each{ |unit|
          ...
          if !@apply_balancing && @cluster_metrics.all?{ |cm| cm[@cut_symbol].positive? }
            ...
            log "Balancing activated late at iteration #{@iterations} after #{(@percent_assigned_cut_load * 100).round(2)}% of the services", level: :warning if @percent_assigned_cut_load > 0.1
          end
        }
      end
