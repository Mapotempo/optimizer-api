...
module Interpreters
  class PeriodicVisits
    ...
    def get_original_values(original, options)
      old_keys = [:unavailable_work_day_indices, :unavailable_visit_day_indices]
      [original.attributes.keys + options.keys - old_keys + [:unavailable_days]].flatten.each_with_object({}) { |key, data|
