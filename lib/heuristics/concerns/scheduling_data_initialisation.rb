...
module SchedulingDataInitialization
  ...
  def adapt_services_data(vrp)
    ...
    vrp.points.each{ |point|
      same_located_set = @services_data.select{ |_id, data| data[:points_ids].include?(point.id) }.sort_by{ |_id, data| data[:visits_number] }
      ...
      raise OptimizerWrapper::UnsupportedProblemError, 'Same_point_day is not supported if frequences of a set have no common multiplexer' unless exist_common_multiplexer(same_located_set.collect{ |_id, data| data[:heuristic_period] })
  ...
  def reject_group(group)
    group.each{ |id, data|
      (1..data[:visits_number]).each{ |index|
        @candidate_services_ids.delete(id)
        @uninserted["#{id}_#{index}_#{data[:visits_number]}"] = {
          original_service: id,
  ...
  def exist_common_multiplexer(lapses)
    lapses = lapses.compact.uniq
    min_lapse = lapses.min

    multiplexers = lapses.collect{ |lapse|
      (2..min_lapse).select{ |multiplexer|
        (lapse % multiplexer).zero?
      }
    }

    multiplexers.size < 2 || multiplexers.first.any?{ |value| multiplexers.all?{ |set| set.include?(value) } }
  end
end
