          ...
          params {
            requires(:vrp, type: Hash, documentation: {param_type: 'body'}) do
              ...
              requires(:vehicles, type: Array, desc: 'Usually represent a work day of a particular driver/vehicle') do
                ...
                optional :speed_multiplier_area, type: Array[Float], coerce_with: ->(c) { c ? c.split(/;|\|/).collect{ |f| Float(f) } : []}, desc: 'Speed multiplier per area, 0 avoid area. Areas separated with pipes (only available for truck mode at this time).'
