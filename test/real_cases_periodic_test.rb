...
class HeuristicTest < Minitest::Test
  if !ENV['SKIP_REAL_SCHEDULING'] && !ENV['SKIP_SCHEDULING']
    ...
    def test_instance_same_point_day
      ...
      # check consistency with same_point_day option
      violation = 0
      vrp[:services].group_by{ |s| s[:activity][:point][:id] }.each{ |point_id, services_set|
        ...
        days_used = result[:routes].count{ |r| r[:activities].any?{ |stop| stop[:point_id] == point_id } }
        violation += 1 unless days_used <= expected_number_of_days
      }
      assert_equal 1, violation,
                   'There is only one case that justifies using more days than visits number in the same_point_day set'
    end
