...
module DistanceMatrix
  ...
  def compute_need_matrix(&block)
    ...
    if !need_matrix.empty?
      ...
      uniq_need_matrix = Hash[uniq_need_matrix.collect{ |mode, dimensions, options|
        block.call(nil, i += 1, uniq_need_matrix.size, 'compute matrix', nil, nil, nil) if block && !self.dichotomious_process?
  ...
  def need_matrix_time?
    ENV['TEST_DUMP_VRP'].to_s == 'true' || !(services.find{ |service|
  ...
  def need_matrix_distance?
    ENV['TEST_DUMP_VRP'].to_s == 'true' || !vehicles.find(&:need_matrix_distance?).nil?
  end
